...
fn test_load_db_file_header() {
    ...
    fn get_field<'a>(fld: &'a str, input: &'a str) -> Option<&'a str> {
        ...
        re.captures(input)
            .and_then(|cap| cap.name("value").map(|login| login.as_str()))
    }
    ...
    check_field!(
        "Checksum",
        (|val: &str| assert_eq!(u32_from_opt(val), db_file_header.checksum))
    );
    check_field!(
        "Format ulMagic",
        (|val: &str| assert_eq!(u32_from_opt(val), db_file_header.signature))
    );
    check_field!(
        "State",
        (|val: &str| assert_eq!(val, db_state_to_string(db_file_header.database_state)))
    );
    check_field!(
        "Last Consistent",
        (|val: &str| {

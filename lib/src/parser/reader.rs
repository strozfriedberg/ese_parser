...
impl Reader {
    fn load_db_file_header(&mut self) -> Result<ese_db::FileHeader, SimpleError> {
        let mut db_file_header = self.read_struct::<ese_db::FileHeader>(0)?;
        ...
        let backup_file_header =
            self.read_struct::<ese_db::FileHeader>(db_file_header.page_size as u64)?;
...
pub fn load_page_header(reader: &Reader, page_number: u32) -> Result<PageHeader, SimpleError> {
    ...
    if reader.format_revision < ESEDB_FORMAT_REVISION_NEW_RECORD_FORMAT {
        ...
        let common = reader
            .read_struct::<PageHeaderCommon>(page_offset + mem::size_of_val(&header) as u64)?;
...
pub fn load_page_tags(reader: &Reader, db_page: &jet::DbPage) -> Result<Vec<PageTag>, SimpleError> {
...
pub fn load_catalog(reader: &Reader) -> Result<Vec<jet::TableDefinition>, SimpleError> {
    ...
    if number_of_variable_size_data_types > 0 {
        ...
        for _ in 0..number_of_variable_size_data_types {
            ...
            let variable_size_data_type_size: u16 =
                reader.read_struct(offset_ddh + variable_size_data_types_offset as u64)?;
...
pub fn find_first_leaf_page(reader: &Reader, mut page_number: u32) -> Result<u32, SimpleError> {
    ...
    for i in start_i..tbl_def.column_catalog_definition_array.len() {
        ...
        if col.identifier <= 127 {
            ...
            while lls.var_state.current_type < col.identifier {
                let variable_size_data_type_size: u16 =
                    reader.read_struct(lls.offset_ddh + lls.var_state.type_offset as u64)?;
                    ...
                    && (variable_size_data_type_size & 0x8000) == 0
                {
                    ...
                    let var_size =
                        variable_size_data_type_size - lls.previous_variable_size_data_type_size;
...
fn load_tagged_data_linear(
    ...
    if tag_state.remaining_definition_data_size > 0 && col.identifier == tag_state.identifier as u32
    {
        ...
        if tag_state.tagged_data_type_size > 0 {
            ...
            {
                data_type_flags =
                    reader.read_struct(offset_ddh + tagged_data_type_value_offset as u64)?;
...
pub fn load_lv_tag(
        ...
        .intersects(jet::PageTagFlags::FLAG_HAS_COMMON_KEY_SIZE)
    {
        common_page_key_size =
            clean_pgtag_flag(reader, db_page, reader.read_struct::<u16>(offset)?);
        first_word_read = true;
        offset += 2;
    ...
    if (page_tag.size as u64) - (offset - page_tag_offset) == 8 {
        ...
        if page_key.len() == 8 {
            ...
            // }.to_be();
            let segment_offset =
                u32::from_le_bytes(page_key[4..8].try_into().expect("Bad slice length")).to_be();
...
pub fn load_lv_metadata(reader: &Reader, page_number: u32) -> Result<LV_tags, SimpleError> {
    let db_page = jet::DbPage::new(reader, page_number)?;
    let pg_tags = &db_page.page_tags;
